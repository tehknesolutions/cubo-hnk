...
function artifact(status='READY_FOR_HUMAN_REVIEW',ready=status==='READY_FOR_HUMAN_REVIEW'){
  return {
    ...
    assessment:{
      ...
      governance:{automaticPromotion:false,mergeAuthorized:false,stablePromotionAuthorized:false,hnkCanonPromotionAuthorized:false,authority:'READINESS_ASSESSMENT_NOT_PROMOTION_AUTHORITY'},
    },
  };
}

function build(overrides={}){
  return buildRc1HumanPromotionDecision({
    artifact:artifact(),
    decision:'APPROVE_V1_0',
    reviewerLabel:'Release Reviewer',
    reason:'Todos os gates obrigatórios foram revisados.',
    acknowledged:true,
    recordedAt:'2026-09-16T12:34:56.789Z',
    recordId:'HOC-DECISION-TEST-001',
    ...overrides,
  });
}

test('readiness inspection only enables approval for READY_FOR_HUMAN_REVIEW',()=>{
  assert.equal(inspectRc1ReadinessArtifact(artifact()).approveAllowed,true);
...
test('foreign or malformed readiness artifact cannot produce any decision record',()=>{
