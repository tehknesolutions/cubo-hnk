...
export function PromotionReadinessClient(){
  ...
  async function importLedger(event:ChangeEvent<HTMLInputElement>){
    ...
    try{
      ...
      setError(cause instanceof Error?cause.message:'Ledger inválido.');
    }
    event.target.value='';
  }
  ...
  function downloadReadiness(){
    ...
    const evidence={
      ...
      sourceLedger:{
        ...
        ledgerVersion:artifact.ledgerVersion??null,
        importedRecords:(artifact.report as {importedRecords?:number})?.importedRecords??null,
        acceptedRecords:(artifact.report as {acceptedRecords?:number})?.acceptedRecords??null,
      },
      assessment:report,
      governance:{
        ...
        note:'Readiness assessment only. Human approval remains mandatory and external to this artifact.',
      },
    };
    const blob=new Blob([JSON.stringify(evidence,null,2)],{type:'application/json'});
    const url=URL.createObjectURL(blob);
    const anchor=document.createElement('a');
    anchor.href=url;anchor.download=`HOC-V1.0-RC1-PROMOTION-READINESS-${report.status}.json`;anchor.click();URL.revokeObjectURL(url);
  }
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.card}>
      <h2>1. Carregar ledger consolidado</h2>
      <p>Este avaliador aceita somente `HOC-RC1-RELEASE-EVIDENCE-LEDGER/V1` da release `HOC-V1.0-RC1`. O arquivo é processado localmente no navegador.</p>
      <label className={styles.label}>Release Evidence Ledger<input type="file" accept="application/json,.json" onChange={importLedger}/></label>
      <div className={styles.inlineActions}><button type="button" onClick={clear} disabled={!artifact}>Limpar</button><button type="button" onClick={downloadReadiness} disabled={!artifact||!report.sourceLedgerValid}>Baixar Promotion Readiness JSON</button></div>
      {fileName&&<p>Arquivo carregado: <strong>{fileName}</strong></p>}
      {error&&<p className={styles.error}>{error}</p>}
    </section>

    <section className={styles.card}>
      <h2>2. Readiness</h2>
      <div className={statusClass(report.status)}>{report.status}</div>
      <div className={styles.resultGrid}>
        <article className={styles.resultCard}><small>Required PASS</small><strong>{report.required.pass}/{report.required.total}</strong></article>
        <article className={styles.resultCard}><small>PENDING</small><strong>{report.required.pending}</strong></article>
    ...
    {report.blockers.length>0&&<section className={styles.card}>
      <h2>3. Blockers obrigatórios</h2>
      <div className={styles.analysisGrid}>{report.blockers.map(item=><article className={styles.analysisCard} key={item.id}><div className={styles.invalidState}>BLOCKED</div><h3>{item.label}</h3><p>{item.detail}</p></article>)}</div>
    </section>}

    {report.pendingRequirements.length>0&&<section className={styles.card}>
      <h2>4. Evidências obrigatórias pendentes</h2>
      <div className={styles.analysisGrid}>{report.pendingRequirements.map(item=><article className={styles.analysisCard} key={item.id}><div className={styles.invalidState}>PENDING</div><h3>{item.label}</h3><p>{item.detail}</p></article>)}</div>
    </section>}

    <section className={styles.card}>
      <h2>5. Próximas ações</h2>
      {report.nextActions.length===0?<p>Nenhuma ação calculada.</p>:<ol>{report.nextActions.map((action,index)=><li key={`${index}-${action}`}>{action}</li>)}</ol>}
    </section>

    <section className={styles.card}>
      <h2>6. Evidência suplementar</h2>
      {report.supplemental.length===0?<p>Nenhum gate suplementar encontrado.</p>:report.supplemental.map(item=><p key={item.id}><strong>{item.label}:</strong> {item.status} — {item.detail}</p>)}
      <p>O Independent Executor é evidência adicional. Ele não substitui o gate obrigatório de GitHub CI.</p>
    </section>
