...
test('validation sequence includes install tests typecheck check build and runtime self-test',()=>{
  const ordered=[
...
test('independent validation gate remains pending until real evidence exists and CI stays blocked',()=>{
  assert.equal(gate('independentValidation').status,'PENDING');
  assert.equal(gate('independentValidation').level,'INSTRUMENTED');
