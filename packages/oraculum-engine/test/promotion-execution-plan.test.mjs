...
test('V16 stack is valid parent-first input for promotion planning',()=>{
  const inspection=inspectRc1StackLandingPlan(stack);
  assert.equal(inspection.valid,true);
  assert.equal(inspection.parentFirst,true);
  assert.equal(inspection.planVersion,'HOC-V1.0-RC1-STACK-LANDING/V16');
  assert.equal(inspection.prCount,33);
  assert.equal(inspection.lastPr,34);
});
...
test('promotion execution plan stays dry-run, fingerprinted and non-executable',()=>{
  ...
  assert.equal(plan.sourceStack.prCount,33);
  assert.equal(plan.sourceStack.lastPr,34);
  assert.equal(plan.summary.stackLandingSteps,33);
  assert.equal(plan.summary.totalSteps,41);
