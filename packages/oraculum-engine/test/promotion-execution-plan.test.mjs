...
test('V17 stack is valid parent-first input for promotion planning',()=>{
  const inspection=inspectRc1StackLandingPlan(stack);
  assert.equal(inspection.valid,true);
  assert.equal(inspection.parentFirst,true);
  assert.equal(inspection.planVersion,'HOC-V1.0-RC1-STACK-LANDING/V17');
  assert.equal(inspection.prCount,34);
  assert.equal(inspection.lastPr,35);
});
...
test('promotion execution plan stays dry-run, fingerprinted and non-executable',()=>{
  ...
  assert.equal(plan.sourceStack.prCount,34);
  assert.equal(plan.sourceStack.lastPr,35);
  assert.equal(plan.summary.stackLandingSteps,34);
  assert.equal(plan.summary.totalSteps,42);
...
test('CLI is local-file dry-run only and contains no mutation/network executor',()=>{
  ...
  assert.match(cli,/DRY_RUN_ONLY|--decision/);
