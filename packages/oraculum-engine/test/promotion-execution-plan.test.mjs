...
import {buildRc1PromotionExecutionPlan,inspectRc1StackLandingPlan,verifyRc1PromotionExecutionPlan,RC1_PROMOTION_EXECUTION_PLAN_VERSION,RC1_PROMOTION_PLAN_FINGERPRINT_VERSION} from '../../../scripts/lib/rc1-promotion-execution-plan.mjs';
...
test('V12 stack is valid parent-first input for promotion planning',()=>{
  const inspection=inspectRc1StackLandingPlan(stack);
  assert.equal(inspection.valid,true);
  assert.equal(inspection.parentFirst,true);
  assert.equal(inspection.planVersion,'HOC-V1.0-RC1-STACK-LANDING/V12');
  assert.equal(inspection.prCount,29);
  assert.equal(inspection.lastPr,30);
});

test('promotion execution plan stays dry-run, fingerprinted and non-executable',()=>{
  ...
  assert.equal(RC1_PROMOTION_PLAN_FINGERPRINT_VERSION,'HOC-RC1-PROMOTION-PLAN-FINGERPRINT/V1');
  assert.equal(plan.releaseId,'HOC-V1.0-RC1');
  assert.equal(plan.mode,'DRY_RUN_ONLY');
  assert.equal(plan.sourceStack.prCount,29);
  assert.equal(plan.sourceStack.lastPr,30);
  assert.equal(plan.summary.stackLandingSteps,29);
  assert.equal(plan.summary.totalSteps,37);
...
test('non-approved, incoherent or forged human decision cannot generate an execution plan',()=>{
  ...
  const forgedMatrix=approvedDecision(); forgedMatrix.sourceReadiness.required={total:9,pass:8,pending:1,blocked:0,missing:0,invalid:0};
  assert.throws(()=>buildRc1PromotionExecutionPlan({decisionRecord:forgedMatrix,stackPlan:stack}),/APPROVE_V1_0/);
  const forgedSource=approvedDecision(); forgedSource.sourceReadiness.evidenceKind='OTHER';
...
test('unsafe or reordered stack cannot generate an execution plan',()=>{
  ...
  const broken=structuredClone(stack); broken.orderedPullRequests[1].base='wrong-parent';
