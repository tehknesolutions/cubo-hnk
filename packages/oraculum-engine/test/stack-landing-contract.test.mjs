...
test('stack landing order is frozen parent-first through human decision record',()=>{
...
test('execution remains blocked while readiness and human-decision instrumentation are implemented',()=>{
  ...
  assert.match(byId.humanDecisionRecorderInstrumentation.reason,/no merge\/deploy\/version\/HNK_CANON action/i);
  assert.equal(byId.vercelPreviewBootstrap.status,'PASS');
});

test('landing rules preserve separation between human decision, readiness, CI and execution',()=>{
