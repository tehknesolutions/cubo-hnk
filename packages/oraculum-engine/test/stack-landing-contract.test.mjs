...
test('stack landing order is frozen parent-first through technical execution authorization',()=>{
...
test('execution remains blocked while authorization instrumentation is implemented',()=>{
  ...
  assert.match(byId.technicalExecutionAuthorizationInstrumentation.reason,/STEP_SCOPED/);
  assert.match(byId.technicalExecutionAuthorizationInstrumentation.reason,/no real authorization record/i);
  assert.equal(byId.vercelPreviewBootstrap.status,'PASS');
});

test('landing rules preserve separation between authorization record, executor, CI and HNK_CANON',()=>{
  ...
  assert.match(rules,/HNK_CANON remains outside Technical Execution Authorization V1/i);
