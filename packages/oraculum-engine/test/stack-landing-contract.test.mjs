...
test('stack landing order is frozen parent-first through mutation evidence verification',()=>{
...
test('evidence verification instrumentation passes without authorizing state mutation',()=>{
  ...
  assert.match(byId.mutationEvidenceVerificationInstrumentation.reason,/eligibleForCompletedPrefix=true/);
  assert.match(byId.mutationEvidenceVerificationInstrumentation.reason,/mutatesCompletedPrefix=false/);
});

test('landing rules keep verification separate from completed-prefix state mutation and HNK_CANON',()=>{
  ...
  assert.match(rules,/Technical Mutation Receipt SUCCESS remains unverified/i);
  assert.match(rules,/eligibleForCompletedPrefix=true/);
  assert.match(rules,/mutatesCompletedPrefix=false/);
